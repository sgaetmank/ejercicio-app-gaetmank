...
export default function App() {
  ...
  async function addPerson() {
    ...
    if (!Number.isInteger(parsedAge) || parsedAge < 0) {
      Alert.alert('Edad invalida', 'Ingresa un numero entero mayor o igual a 0');
      return;
    }
  ...
  // Ejecuta el DELETE real contra la API una vez confirmado.
  async function performDelete(person: Person) {
    const response = await fetch(`${API_URL}/api/personas/${person.id}`, {
      method: 'DELETE',
    });
    ...
    // Un 204 no trae body: no hay nada que leer con response.json().
    await loadPeople();
  }


  function deletePerson(person: Person) {
    const message = `Seguro que queres eliminar a ${person.firstName} ${person.lastName}?`;
    ...
    Alert.alert('Eliminar registro', message, [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: () => performDelete(person) },
    ]);
  }
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.personCard}>
            ...
            <Text style={styles.personInfo}>| Edad: {item.age}</Text>
...
const styles = StyleSheet.create({
  ...
  deleteButton: { marginTop: 8, padding: 9, borderWidth: 1, borderColor: '#7a3b3b', backgroundColor: '#301919' },
  deleteButtonText: { color: '#e0a3a3', textAlign: 'center', fontFamily: 'monospace', fontSize: 12 },
});
